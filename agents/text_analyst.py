...
Text Analysis Agent
-------------------
Given a hotel's reviews and a parsed query, uses the LLM to:
...
SYSTEM_PROMPT = """You are a hotel review analyst. Your job is to carefully read hotel reviews
...
async def analyze_reviews(hotel_name: str, reviews: list[dict], parsed_query: dict) -> dict:
    ...
    raw = await chat(
        ...
        user_content=f"""Hotel: {hotel_name}
